...
Goal
- Reliability + safety of the rebuild (plan â†’ execute â†’ undo manifest).
...
class StatCard(tk.Frame):
    def __init__(self, parent, title: str, value: str, accent: str, **kwargs):
        ...
        tk.Label(top, text="â—", bg=Theme.BG_PANEL, fg=accent, font=Theme.FONT_SMALL).pack(side=tk.RIGHT)
...
class LogView(tk.Frame):
    ...
    def add(self, message: str, level: str = "info") -> None:
        ...
        tk.Label(row, text=("â—" if level in ("success", "error", "warning") else "Â·"), bg=Theme.BG_CARD, fg=color, font=Theme.FONT_MONO_S).pack(
            side=tk.LEFT, padx=4
        )
...
class NexusApp(TkinterDnD.Tk if HAS_DND else tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        self.card_files = StatCard(stats, "Planned Moves", "â€”", Theme.CYAN)
        self.card_files.grid(row=0, column=0, sticky="ew", padx=(0, Theme.S2))
        self.card_cats = StatCard(stats, "Categories", "â€”", Theme.PURPLE)
        self.card_cats.grid(row=0, column=1, sticky="ew", padx=(Theme.S2, Theme.S2))
        self.card_dupes = StatCard(stats, "Duplicate Sets", "â€”", Theme.WARNING)
        self.card_dupes.grid(row=0, column=2, sticky="ew", padx=(Theme.S2, Theme.S2))
        self.card_ignored = StatCard(stats, "Ignored", "â€”", Theme.TEXT_MUTED)
    ...
    def _sync_hero_state(self) -> None:
        ...
        if hasattr(self, "pill_cloud"):
            self.pill_cloud.set_text(f"CLOUD SAFE Â· {STATE.batch_limit}")
        ...
        if hasattr(self, "analyze_progress"):
            try:
                ...
                if hasattr(self, "analyze_progress_label"):
                    self.analyze_progress_label.config(text="Preparing analysisâ€¦")
            except Exception:
                pass
        ...
        try:
            ...
            if hasattr(self, "analyze_progress_label"):
                ...
                self.analyze_progress_label.config(
                    text=f"Analyzingâ€¦ {done:,} / {total:,} files ({pct}%)"
                )
        except Exception:
            pass
    ...
    def clear_plan(self) -> None:
        ...
        self._set_status("Plan cleared â€” ready for new analysis")
