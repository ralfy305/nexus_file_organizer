...
class AppController:
    ...
    def source_paths(self, exists_only: bool = False) -> list[Path]:
        paths = [Path(s) for s in self.state.sources]
        return [p for p in paths if p.exists()] if exists_only else paths

    def add_source(self, path: Path) -> str | None:
        resolved = str(path.resolve())
        if resolved in self.state.sources:
            return None
        self.state.sources.append(resolved)
        save_config()
        return resolved

    def remove_source_at(self, index: int) -> str:
        removed = self.state.sources.pop(index)
        save_config()
        return removed

    def set_destination(self, path: Path) -> Path:
        resolved = validate_destination(path, self.source_paths()).resolve()
        validate_cloud_placement(
            self.source_paths(exists_only=True),
            resolved,
            cloud_safe_mode=self.state.cloud_safe_mode,
        )
        self.state.destination = str(resolved)
        save_config()
        return resolved

    def set_rules_text(self, text: str) -> str:
        self.state.rules_text = text
        save_config()
        return self.state.rules_text

    def update_preferences(self, **changes) -> None:
        for key, value in changes.items():
            setattr(self.state, key, value)
        save_config()

    def store_plan(self, plan: Plan) -> None:
        self.state.last_plan = plan

    def last_plan(self) -> Plan:
        return self.state.last_plan

    def store_manifest(self, manifest: Path | None) -> None:
        self.state.last_manifest = manifest

    def last_manifest(self) -> Path | None:
        return self.state.last_manifest

    def post_execute_empty_dirs(self) -> list[Path]:
        empty_dirs: list[Path] = []
        for source in self.source_paths():
            empty_dirs.extend(find_empty_dirs(source, keep_root=True))
        return empty_dirs
...
def parse_dropped_paths(widget: tk.Misc, raw_data: str) -> list[str]:
    if not raw_data:
        return []
    try:
    ...
    cleaned: list[str] = []
    for part in parts:
        path = str(part).strip().strip("{}").strip('"')
        if path:
            cleaned.append(path)
    return cleaned
...
class DuplicateReviewer(tk.Toplevel):
    def __init__(self, parent: "NexusApp", plan: Plan):
        ...
        self.keep_vars: dict[str, tk.StringVar] = {}
        ...
        for index, (group_key, paths) in enumerate(self.groups, start=1):
            ...
            tk.Label(
                card,
                text=f"Duplicate Set {index} ({len(paths)} files)",
                bg=Theme.BG_CARD,
                fg=Theme.WARNING,
                font=Theme.FONT_H3,
            ).pack(anchor="w")
            var = tk.StringVar(value=str(paths[0]))
            self.keep_vars[group_key] = var
            for path in paths:
                ...
                badge = tk.Label(row, text=icon_for_path(path), bg=Theme.INDIGO, fg=Theme.TEXT_PRIMARY, font=Theme.FONT_SMALL, padx=8, pady=4)
                ...
                tk.Label(
                    row,
                    text=f"{meta['size']}   Modified {meta['mtime']}",
                    bg=Theme.BG_PANEL,
                    fg=Theme.TEAL,
                    font=Theme.FONT_SMALL,
                    anchor="w",
    ...
    def apply_review(self) -> None:
        keep_map = {group: Path(var.get()) for group, var in self.keep_vars.items()}
        apply_duplicate_selection(self.plan, keep_map)
...
class NexusApp(TkinterDnD.Tk if HAS_DND else tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        self.include_dupes_var = tk.BooleanVar(value=STATE.include_duplicates)
        self.organize_mode_var = tk.StringVar(value=STATE.organize_mode)
        self.cloud_safe_var = tk.BooleanVar(value=STATE.cloud_safe_mode)
        self.batch_limit_var = tk.StringVar(value=str(STATE.batch_limit))
        ...
        tk.Checkbutton(
            toggle_frame,
            text="Include duplicates in moves",
            variable=self.include_dupes_var,
            command=self.toggle_include_duplicates,
            bg=Theme.BG_PANEL,
            fg=Theme.TEXT_PRIMARY,
            selectcolor=Theme.BG_DARK,
            activebackground=Theme.BG_PANEL,
        ...
        tk.Checkbutton(
            toggle_frame,
            text="Cloud-safe mode",
            variable=self.cloud_safe_var,
            command=self.toggle_cloud_safe_mode,
            bg=Theme.BG_PANEL,
            fg=Theme.TEXT_PRIMARY,
            selectcolor=Theme.BG_DARK,
            activebackground=Theme.BG_PANEL,
        ...
        mode_menu = ttk.Combobox(toggle_frame, textvariable=self.organize_mode_var, values=("category", "mirror", "folder_sort"), state="readonly")
        mode_menu.pack(fill=tk.X)
        mode_menu.bind("<<ComboboxSelected>>", lambda _event: self.toggle_organize_mode())
        ...
        batch_menu = ttk.Combobox(toggle_frame, textvariable=self.batch_limit_var, values=("100", "250", "500", "1000"), state="readonly")
        batch_menu.pack(fill=tk.X)
        batch_menu.bind("<<ComboboxSelected>>", lambda _event: self.toggle_batch_limit())
        ...
        tk.Label(status_shell, textvariable=self.status_var, bg=Theme.BG_PANEL, fg=Theme.TEXT_SECONDARY, wraplength=260, justify="left").pack(anchor="w", pady=(4, 0))
        ...
        self.summary_var = tk.StringVar(value="No execution yet. Analyze a plan, review duplicates, and run when ready.")
        tk.Label(self.summary_card, textvariable=self.summary_var, bg=Theme.BG_PANEL, fg=Theme.TEXT_SECONDARY, font=Theme.FONT_BODY, justify="left", wraplength=1000).grid(row=1, column=1, sticky="w")
        ...
        self.destination_label = tk.Label(dst_card, textvariable=self.destination_var, bg=Theme.BG_DARK, fg=Theme.TEXT_PRIMARY, wraplength=520, justify="left", padx=12, pady=12, highlightbackground=Theme.BORDER_SOFT, highlightthickness=1)
    ...
    def _handle_source_drop(self, event) -> None:
        added = 0
        for path_text in parse_dropped_paths(self, event.data):
            path = Path(path_text)
            if path.is_dir():
                resolved = str(path.resolve())
    ...
    def _handle_destination_drop(self, event) -> None:
        ...
        try:
            ...
            return
        STATE.destination = str(resolved)
        save_config()
        self._refresh_destination()
    ...
    def _refresh_destination(self) -> None:
        self.destination_var.set(STATE.destination or "No destination selected")
    ...
    def _refresh_toggles(self) -> None:
        ...
        self.include_dupes_var.set(STATE.include_duplicates)
        self.organize_mode_var.set(STATE.organize_mode)
        self.cloud_safe_var.set(STATE.cloud_safe_mode)
        self.batch_limit_var.set(str(STATE.batch_limit))
        self._sync_hero_state()

    def toggle_dry_run(self) -> None:
        STATE.dry_run = bool(self.dry_run_var.get())
        save_config()
        self._sync_hero_state()
        self._log(f"Dry Run set to {STATE.dry_run}", "info")

    def toggle_include_duplicates(self) -> None:
        STATE.include_duplicates = bool(self.include_dupes_var.get())
        save_config()
        self._sync_hero_state()
        self._log(f"Include duplicates set to {STATE.include_duplicates}", "info")

    def toggle_cloud_safe_mode(self) -> None:
        STATE.cloud_safe_mode = bool(self.cloud_safe_var.get())
        save_config()
        self._sync_hero_state()
    ...
    def toggle_batch_limit(self) -> None:
        try:
            STATE.batch_limit = max(1, int(self.batch_limit_var.get()))
        except ValueError:
            STATE.batch_limit = 500
            self.batch_limit_var.set(str(STATE.batch_limit))
        save_config()
        self._sync_hero_state()
    ...
    def toggle_organize_mode(self) -> None:
        STATE.organize_mode = str(self.organize_mode_var.get())
        save_config()
        self._sync_hero_state()
    ...
    def _sync_summary(self) -> None:
        ...
        mode = summary["mode"]
        if mode == "idle":
            self.summary_var.set("No execution yet. Analyze a plan, review duplicates, and run when ready.")
            return
        if mode == "cancelled":
            self.summary_var.set(f"Run cancelled after {summary['elapsed']:.1f}s. Moves completed: {summary['moves']}, duplicates trashed: {summary['trashed']}, errors: {summary['errors']}.")
            return
        label = "Dry run" if mode == "dry_run" else "Live run"
        self.summary_var.set(f"{label} finished in {summary['elapsed']:.1f}s. Moves: {summary['moves']}, duplicates trashed: {summary['trashed']}, errors: {summary['errors']}.")
    ...
    def add_source(self) -> None:
        ...
        if not path:
            return
        added = self.controller.add_source(Path(path))
        if added:
            self._refresh_sources()
            self._log(f"Added source: {added}", "success")
    ...
    def start_analyze(self) -> None:
        ...
        STATE.rules_text = self._collect_rules_text()
        save_config()
